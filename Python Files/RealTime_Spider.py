...
def update_dashboard():
    ...
    for year in range(2010, 2028):
        ...
        if year > 2017:
            ...
            synthetic_data = pd.DataFrame({
                'Unintentional Release (Barrels)': np.random.uniform(10, 50, size=10),
                'Liquid Recovery (Barrels)': [liquid_recovery] * 10,
                'Net Loss (Barrels)': [net_loss] * 10,
                'Property Damage Costs': np.random.rand(10) * 10000,
                'Lost Commodity Costs': np.random.rand(10) * 5000,
                'Public/Private Property Damage Costs': np.random.rand(10) * 3000,
                'Emergency Response Costs': np.random.rand(10) * 2000,
                'Environmental Remediation Costs': np.random.rand(10) * 10000,
                'Other Costs': np.random.rand(10) * 1000,
                'Accident Longitude': np.random.uniform(-100, -80, 10),
                'Accident Latitude': np.random.uniform(25, 50, 10)
            })
